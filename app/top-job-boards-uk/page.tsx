...
export default function JobBoardsPage() {
  return <EditorialGuideShell path={path} eyebrow="UK job-search guide" title="The best UK job sites—compared by what they are actually useful for." intro="You came for places to find jobs, so this page starts with real job sites. Use the comparison to choose a small search stack, then follow the workflow to avoid duplicate listings, stale adverts and rushed applications." answer="For most people, start with Indeed or another broad board, add LinkedIn for professional visibility, and use the official or specialist site for your sector. Save target employers separately. One relevant alert is worth more than hundreds of unfiltered results." reviewed={displayReviewDate(customerContentReview[path])} faqs={faqs} ctaHeading="Found a suitable vacancy? Build the CV for that vacancy." ctaBody="Use the advert as your brief, tailor your evidence and preview the finished UK CV. WorkCV charges once only if you choose to download the PDF—no monthly subscription." trackingContext="job_boards_editorial" relatedLinks={[["Job Application Pack", "/tools/job-application-pack-uk"], ["How to prepare for an interview", "/how-to-prepare-for-a-job-interview-uk"], ["Customer service CV example", "/cv-template-customer-service-uk"], ["Driver CV example", "/cv-template-driver-uk"], ["No-subscription CV builder", "/cv-builder-no-subscription-uk"]]}>
    ...
    <GuideSection label="Quick comparison" title="Seven broad UK job sites and when to use each." intro="These are not presented as a made-up league table. Each has a different role in a search; open the sites that match your situation and judge the live results for yourself.">
      <div className="grid gap-3 sm:hidden">{generalBoards.map(board => <article key={board.name} className="rounded-lg border border-line bg-white p-5"><h3 className="font-display text-xl font-semibold text-navy"><a className="underline" href={board.href} target="_blank" rel="noreferrer">{board.name}<span className="sr-only"> (opens in a new tab)</span></a></h3><p className="mt-3 text-sm"><strong>Best for:</strong> {board.best}</p><p className="mt-2 text-sm leading-6"><strong>Why it can help:</strong> {board.useful}</p><p className="mt-2 text-sm leading-6"><strong>Watch for:</strong> {board.watch}</p></article>)}</div>
      <div className="hidden overflow-x-auto rounded-lg border border-line bg-white shadow-sm sm:block" role="region" aria-label="Seven broad UK job sites comparison" tabIndex={0}><table className="w-full min-w-[820px] border-collapse text-left text-sm"><caption className="sr-only">Seven broad UK job sites and their uses</caption><thead className="bg-navy text-white"><tr><th scope="col" className="p-4">Job site</th><th scope="col" className="p-4">Best for</th><th scope="col" className="p-4">Why it can help</th><th scope="col" className="p-4">Watch for</th></tr></thead><tbody>{generalBoards.map((board) => <tr key={board.name} className="border-t border-line align-top"><th scope="row" className="p-4"><a className="font-bold text-navy underline" href={board.href} target="_blank" rel="noreferrer">{board.name}<span className="sr-only"> (opens in a new tab)</span></a></th><td className="p-4 font-semibold text-navy">{board.best}</td><td className="p-4 leading-6 text-muted">{board.useful}</td><td className="p-4 leading-6 text-muted">{board.watch}</td></tr>)}</tbody></table></div>
    </GuideSection>
    <section className="border-b border-line bg-paper py-16"><div className="container-page max-w-6xl">
      <CustomerAnswer questionId="Q22">
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
          {[
            { name: "Prospects graduate jobs", href: "https://www.prospects.ac.uk/graduate-jobs", audience: "Graduate opportunities", action: "Search a target role or sector, then check eligibility and closing date.", limit: "Not every first job is a graduate scheme." },
            { name: "Gradcracker", href: "https://www.gradcracker.com/", audience: "STEM students and graduates", action: "Choose your discipline and opportunity type; inspect the employer's requirements.", limit: "Focused on STEM, not general retail or admin roles." },
            { name: "Indeed UK", href: "https://uk.indeed.com/", audience: "Broad role discovery", action: "Try a real title such as junior administrator, a town and a recent-posting filter if available.", limit: "Entry level labels do not guarantee no experience is required." },
            { name: "LinkedIn Jobs", href: "https://www.linkedin.com/jobs/", audience: "Broad discovery and employer research", action: "Search role and location; inspect where Apply leads.", limit: "Listings may redirect and account requirements vary." },
            { name: "Employer careers pages", href: "https://www.gov.uk/find-a-job", audience: "Specific target employers", action: "Navigate from each employer's verified official website; match role, reference and location.", limit: "The GOV.UK link is a separate official search starting point, not a directory of every employer's careers page." },
          ].map((board) => <article key={board.name} className="rounded-lg border border-line bg-white p-5"><h3 className="font-display text-xl font-semibold text-navy">{board.name}</h3><p className="mt-2 text-sm"><strong>For:</strong> {board.audience}</p><p className="mt-2 text-sm"><strong>Try:</strong> {board.action}</p><p className="mt-2 text-sm text-muted"><strong>Limit:</strong> {board.limit}</p>{board.name === "Employer careers pages" ? <p className="mt-3 text-sm">Use each employer's own official site. <a className="font-semibold text-navy underline" href={board.href} target="_blank" rel="noreferrer">GOV.UK Find a job</a> is another official route.</p> : <a className="mt-3 inline-block font-semibold text-navy underline" href={board.href} target="_blank" rel="noreferrer">Open {board.name}</a>}</article>)}
        </div>
        <p><strong>Illustrative search plan:</strong> A Leeds graduate seeking junior operations or admin work checks Prospects, one broad board and three target employers. They search “graduate operations”, “junior administrator” and “operations assistant”, set a realistic commute, read essential criteria and record the closing date, vacancy reference and actual application URL. These examples are search ideas, not claims that a matching vacancy is live today.</p>
        <p>Once you have one suitable advert, <TrackedLink href="/tools/job-application-pack-uk" placement={analyticsPlacements.customerQ22Pack} className="font-semibold text-navy underline">map its requirements to your evidence</TrackedLink> before editing your CV.</p>
      </CustomerAnswer>
    </div></section>
    ...
    <section className="border-b border-line bg-surface py-16"><div className="container-page max-w-6xl">
      <CustomerAnswer questionId="Q23">
        <ol className="list-decimal space-y-2 pl-6"><li>From the board listing, record the employer, exact title, location, vacancy reference, deadline and application instructions.</li><li>Independently open the employer's verified careers page and compare those details where a direct listing exists.</li><li>Follow the stated application route. If the board redirects to the employer, complete that process; avoid submitting the same application twice unless instructed.</li><li>Keep the advert, the CV version you sent and the date of submission for interview preparation.</li></ol>
        <p><strong>Illustrative example:</strong> A board advert says “Operations Assistant, Leeds, reference OA-26”. Check the employer's official careers site for that reference and deadline before following its Apply instructions. If you cannot find it, verify through official employer contact details; absence alone does not prove the board advert is fraudulent.</p>
        <p>Use the <Link href="/tools/job-application-pack-uk" className="font-semibold text-navy underline">application pack</Link> to review the verified advert against your actual CV. No success-rate comparison between board and direct routes is claimed.</p>
      </CustomerAnswer>
    </div></section>
