...
} from "@/lib/cv-bullet-point-generator";
...
export async function POST(request: NextRequest) {
  try {
    ...
    const input = cvBulletPointInputSchema.parse(JSON.parse(body));
    ...
    return NextResponse.json(await generateCvBulletPoints(input), { headers });
  } catch (error) {
    ...
    if (error instanceof CvBulletPointError) {
      return NextResponse.json(
        { error: error.message },
    ...
    console.error("workcv_cv_bullets_error", {
