...
const categories: HubCategory[] = [
  {
    ...
    icon: Briefcase,
    cards: [
      { title: "Top job boards in the UK", href: "/top-job-boards-uk", description: "Choose a board by sector, location, seniority and working pattern." },
  ...
  },
  {
    ...
    icon: FileCheck2,
    cards: [
      { title: "Prepare for a job interview", href: "/how-to-prepare-for-a-job-interview-uk", description: "Use the advert, your CV and a simple answer plan to prepare." },
      { title: "Common interview questions", href: "/common-job-interview-questions-uk", description: "Plan truthful examples for general, competency and role-specific questions." },
      { title: "Thank-you email after an interview", href: "/thank-you-email-after-interview-uk", description: "Use a concise UK structure without overclaiming or sounding generic." },
  ...
  },
  {
    ...
    icon: TrendingUp,
    cards: [
      { title: "Career advancement strategies", href: "/career-advancement-strategies-uk", description: "Turn goals, achievements and target-role language into an actionable plan." },
...
export function CareerToolsHub() {
  ...
  return <>
    ...
    <section className="quiet-grid border-b border-line bg-paper py-16 md:py-24">
      <div className="container-page grid items-center gap-12 lg:grid-cols-[1.1fr_0.9fr]">
        <div>
          ...
          <h1 className="max-w-4xl font-display text-5xl font-semibold leading-[0.98] text-navy md:text-7xl">From a trending question to a better application.</h1>
          <p className="mt-7 max-w-3xl text-lg leading-8 text-muted md:text-xl">Practical UK guides and tools for finding a role, preparing the application and turning your real experience into a CV you can stand behind.</p>
          <div className="mt-8 flex flex-col gap-3 sm:flex-row">
            <TrackedLink href={commercialRoutes.moneyPage} placement={analyticsPlacements.careerToolsHubMoneyHero} className="inline-flex min-h-12 items-center justify-center gap-2 rounded-md bg-navy px-6 py-3 text-sm font-bold text-white shadow-sm transition hover:-translate-y-0.5 hover:bg-navy-hover">Build the CV behind your next application <ArrowRight className="h-4 w-4" /></TrackedLink>
            <Link href="/tools/job-application-pack-uk" className="inline-flex min-h-12 items-center justify-center gap-2 rounded-md border border-line-strong bg-white px-6 py-3 text-sm font-bold text-navy transition hover:-translate-y-0.5">Try the Job Application Pack <ArrowRight className="h-4 w-4" /></Link>
          </div>
        </div>
        <aside className="rounded-2xl border border-line bg-white p-7 shadow-sm md:p-9">
          ...
          <ol className="mt-6 grid gap-5">
            {["Find the question behind the search", "Use the vacancy and your evidence together", "Edit the draft until it is truthful", "Preview a clear UK CV before you send it"].map((item, index) => <li key={item} className="flex gap-4"><span className="inline-flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-navy text-sm font-bold text-white">{index + 1}</span><span className="pt-1 text-sm leading-6 text-muted">{item}</span></li>)}
          </ol>
        </aside>
      </div>
    </section>
    ...
    <MoneyPageCta heading="Build the CV behind the trend." body={`Once you know what the vacancy needs, build and preview your UK CV, then pay ${site.price} once only if you download the finished PDF.`} trackingContext="career_tools_hub" />
