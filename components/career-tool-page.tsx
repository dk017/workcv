...
import { DocumentReadabilityChecker } from "@/components/document-readability-checker";
...
type CareerToolPageProps = {
  ...
  afterToolContent?: ReactNode;
};

export function CareerToolPage({ path, tool, eyebrow, title, intro, description, howItHelps, links, faqs, afterToolContent }: CareerToolPageProps) {
  const schemas = [
...
function ToolSwitch({ tool }: { tool: CareerToolId }) {
  if (tool === "format") return <><DocumentReadabilityChecker /><CvFormatCheckerTool /></>;
